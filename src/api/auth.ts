...
export const authService = {
  async register(name: string, email: string): Promise<AuthResponse> {
    try {
      const response = await axios.post(`${API_BASE_URL}/auth/register`, { name, email });
  ...
  async requestVerificationCode(email: string): Promise<void> {
    try {
      ...
      const axiosError = error as AxiosError<ErrorResponse>;
      throw new Error(axiosError.response?.data.message || 'Failed to request verification code');
    }
  },
  ...
  async verifyCode(email: string, code: string): Promise<AuthResponse> {
    try {
      ...
      const axiosError = error as AxiosError<ErrorResponse>;
      throw new Error(axiosError.response?.data.message || 'Verification failed');
    }
  },

  async login(email: string, code: string): Promise<AuthResponse> {
    try {
      const response = await axios.post(`${API_BASE_URL}/auth/login`, { email, code });
      return response.data.data;
    } catch (error) {
      const axiosError = error as AxiosError<ErrorResponse>;
